...
export default function DoctorDashboard() {
	...
	const fetchDoctorData = async () => {
		try {
			...
			// Check if doctor record doesn't exist (404 error)
			const errorMessage = (error as any)?.response?.status === 404 || (error as any)?.response?.data?.error?.includes("not found") || (error as any)?.message?.includes("404");
		...
		<>
			<div className="flex items-center justify-between space-y-2 mb-4">
				...
				<div className="flex items-center space-x-2">
					<Button onClick={() => router.push("/doctor/dashboard/appointments")}>View Appointments</Button>
				</div>
			</div>
			<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 mb-4">
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold">{doctorMetricsLoading ? "..." : doctorMetrics?.totalAppointments?.count || 0}</div>
						<p className="text-xs text-muted-foreground">{doctorMetricsLoading ? "Loading..." : doctorMetrics?.totalAppointments?.growthText || "+0 from last month"}</p>
					</CardContent>
				</Card>
				<Card>
					<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
					...
					<CardContent>
						<div className="text-2xl font-bold">{doctorMetricsLoading ? "..." : doctorMetrics?.todaySchedule.count || 0}</div>
						<p className="text-xs text-muted-foreground">{doctorMetricsLoading ? "Loading..." : doctorMetrics?.todaySchedule.text || "appointments today"}</p>
					</CardContent>
				</Card>
				<Card>
					<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
					...
					<CardContent>
						<div className="text-2xl font-bold">{doctorMetricsLoading ? "..." : `$${doctorMetrics?.revenue.total || 0}`}</div>
						<p className="text-xs text-muted-foreground">{doctorMetricsLoading ? "Loading..." : doctorMetrics?.revenue.growthText || "+$0 from last month"}</p>
					</CardContent>
				</Card>
			</div>
